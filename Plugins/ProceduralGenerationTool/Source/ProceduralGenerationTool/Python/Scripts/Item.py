...
class Item:
    def __init__(self, name, length, width, height, weight):
        ...
        self.position = AuxiliaryMethods.START_POSITION # initial position: (0, 0, 0)
    ...
    def format_numbers(self, number_of_decimals):
        ...
        self.weight = AuxiliaryMethods.set_to_decimal(self.weight, number_of_decimals)
    ...
    def get_dimension(self): # 6 orientation types -- (x-axis, y-axis, z-axis)
        if self.rotation_type == Constants.RotationType.RT_LWH:
            dimension = [self.length, self.width, self.height]
        elif self.rotation_type == Constants.RotationType.RT_HLW:
            dimension = [self.height, self.length, self.width]
        elif self.rotation_type == Constants.RotationType.RT_HWL:
            dimension = [self.height, self.width, self.length]
        elif self.rotation_type == Constants.RotationType.RT_WHL:
            dimension = [self.width, self.height, self.length]
        elif self.rotation_type == Constants.RotationType.RT_WLH:
            dimension = [self.width, self.length, self.height]
        elif self.rotation_type == Constants.RotationType.RT_LHW:
